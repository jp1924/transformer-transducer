...
# [NOTE]: copied from wav2vec2 spec-augment
def _compute_mask_indices(
    ...
    Args:
        ...
        mask_prob:  The percentage of the whole axis (between 0 and 1) which will be masked. The number of
                    ...
                    전체 인덱스 중에서 마스킹을 할 인덱스의 비율을 설정한다. 마스크의 길이는 `mask_prob*shape[1]/mask_length`와 같이 계산된다.
    ...
    def compute_num_masked_span(input_length):
        ...
        num_masked_span = (mask_prob * input_length) // (mask_length + epsilon)
    ...
    # 음성의 길이를 attention마스크 혹은 shape에서 나온 sequence_length를 이용해 값을 측정한다.
...
class TransducerModel(TransformerTransducerPretrainedModel):
    ...
    def spec_augment(
        ...
        if self.config.mask_frequency_prob > 0:
            # generate indices & apply SpecAugment along feature axis
            mask_feature_indices = _compute_mask_indices(
                (batch_size, hidden_size),
                mask_prob=self.config.mask_frequency_prob,
                mask_length=self.config.mask_frequency_length,
            )
