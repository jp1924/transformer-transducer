...
class DataCollatorRNNTWithPadding(DataCollatorMixin):
    ...
    def torch_call(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        input_features = [
            {"input_features": self._mask_hidden_states(feature["input_features"])[0]}
    ...
    def _mask_hidden_states(
        ...
        """
        config = self.model.config.audio_config
        ...
        elif config.mask_time_prob > 0 and self.model.training:
        ...
        if config.mask_feature_prob > 0 and self.model.training:
